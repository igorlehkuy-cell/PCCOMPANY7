...
const products = [
    ...
    { id: 13, name: 'JBL Charge 5', priceUSD: 200, image: 'speaker', type: 'speaker', category: 'office', manufacturer: 'JBL', description: 'Потужний звук, сильний бас і довго тримає батарея — універсальний варіант на кожен день.', specs: ['🔊 40W звук', '🔋 до 20 год', '💧 IP67', '🔌 Powerbank'] },
    { id: 14, name: 'Anker Soundcore Motion X600', priceUSD: 210, image: 'speaker', type: 'speaker', category: 'office', manufacturer: 'JBL', description: 'Чистий об’ємний звук і топ за свою ціну — хороший баланс якості і вартості.', specs: ['🔊 50W звук', '🎵 Hi-Res Audio', '🔋 до 12 год', '🌊 Просторовий звук'] },
    { id: 15, name: 'Bose SoundLink Flex', priceUSD: 150, image: 'speaker', type: 'speaker', category: 'gaming', manufacturer: 'Bose', description: 'Якісний і збалансований звук — більше для тих, хто цінує чистоту музики.', specs: ['\'🔊 чистий звук\', \'🔋 до 12 год\', \'💧 водостійка\', \'🎵 Balanced Audio\''] }
];
...
function createFilterPanel() {
    ...
    <div class="filter-panel-body">
      <div class="filter-group">
        <label>Тип</label>
        <select id="filterTypeSelect"><option value="all">Всі</option><option value="desktop">Комп'ютери</option><option value="laptop">Ноутбуки</option><option value="speaker">Колонки</option><option value="camera">Камери</option></select>
      </div>
      <div class="filter-group">
...
function generateProductCard(product) {
    ...
    <div class="price-tag">${formatUSD(product.priceUSD)}<span class="price-per-day">/день</span></div>
    </div>
    <button class="product-details-btn" data-id="${product.id}">Орендувати</button>
  `;
...
// --- Product modal (Rental version with days picker) ---
function createProductModal() {
    ...
    <div class="modal-content">
        ...
        <div class="modal-section">
          ...
          <div class="rental-pricing">
            <div class="rental-price-row">
              <span class="rental-label">Ціна за день:</span>
              <span class="rental-day-price" id="modalDayPrice">$0</span>
            </div>
            <div class="rental-price-row">
              <span class="rental-label">Кількість днів:</span>
              <div class="rental-days-control">
                <button class="rental-days-btn" id="rentalDaysDown">−</button>
                <input type="number" id="rentalDays" value="1" min="1" max="365">
                <button class="rental-days-btn" id="rentalDaysUp">+</button>
              </div>
            </div>
            <div class="rental-price-row rental-total-row">
              <span class="rental-label">Загальна вартість:</span>
              <span class="rental-total-price" id="modalTotalPrice">$0</span>
            </div>
          </div>
        </div>
      ...
      <div class="modal-footer"><button class="btn-modal-close">Закрити</button><button class="btn-modal-add-cart" id="modalAddToCartBtn">🛒 Орендувати</button></div>
    </div>
  `;
    ...
    document.getElementById('rentalDaysDown').addEventListener('click', () => {
        const inp = document.getElementById('rentalDays');
        inp.value = Math.max(1, parseInt(inp.value) - 1);
        updateRentalTotal();
    });
    document.getElementById('rentalDaysUp').addEventListener('click', () => {
        const inp = document.getElementById('rentalDays');
        inp.value = Math.min(365, parseInt(inp.value) + 1);
        updateRentalTotal();
    });
    document.getElementById('rentalDays').addEventListener('input', updateRentalTotal);
}

let _currentRentalProductId = null;
let _currentDayPrice = 0;

function updateRentalTotal() {
    const days = Math.max(1, parseInt(document.getElementById('rentalDays').value) || 1);
    document.getElementById('rentalDays').value = days;
    const total = _currentDayPrice * days;
    document.getElementById('modalTotalPrice').textContent = formatUSD(total);
}

function openProductModal(id) {
    createProductModal();
    const product = products.find(p => p.id === id); if (!product) { window.showToast('Товар не знайдено', 'error'); return; }
    _currentRentalProductId = id;
    _currentDayPrice = product.priceUSD;
    document.getElementById('modalProductName').textContent = product.name;
    document.getElementById('modalDayPrice').textContent = formatUSD(product.priceUSD);
    document.getElementById('modalProductType').textContent = product.type === 'desktop' ? 'Комп\'ютер' : product.type === 'laptop' ? 'Ноутбук' : product.type === 'speaker' ? 'Колонка' : 'Пристрій';
    document.getElementById('modalProductIcon').textContent = product.type === 'desktop' ? '🖥️' : product.type === 'laptop' ? '💻' : product.type === 'speaker' ? '🔊' : '📦';
    ...
    addBtn.onclick = () => {
        const days = Math.max(1, parseInt(document.getElementById('rentalDays').value) || 1);
        const totalPrice = product.priceUSD * days;
        addProductToCart(product.id, product.name, product.priceUSD, product.type, days, totalPrice);
        closeProductModal();
    };
...
// --- Cart (Rental version) ---
function addProductToCart(id, name, dayPrice, type, days, totalPrice) {
    const cart = JSON.parse(localStorage.getItem('pc-company-cart') || '[]');
    const existing = cart.find(i => i.id === id);
    if (existing) {
        existing.days = (existing.days || 1) + days;
        existing.totalPrice = existing.dayPrice * existing.days;
        existing.quantity = 1;
    } else {
        cart.push({ id, name, dayPrice, type, days: days || 1, totalPrice: totalPrice || dayPrice, quantity: 1 });
    }
    localStorage.setItem('pc-company-cart', JSON.stringify(cart));
    window.showToast(`✅ ${name} додано до кошика (${days} дн.)!`, 'success');
}
...
function renderCartItems() {
    ...
    if (!list) return;
    if (cart.length === 0) {
        list.innerHTML = '<p class="empty-cart">🛒 Кошик пустий</p>';
    ...
    let items = 0;
    cart.forEach((it, idx) => {
        const dayPrice = parsePrice(it.dayPrice || it.price) || 0;
        const days = it.days || 1;
        const itemTotal = dayPrice * days;
        total += itemTotal;
        items += 1;
        ...
        div.innerHTML = `
            <div class="cart-item-info">
                <div class="cart-item-icon">${it.type === 'desktop' ? '🖥️' : it.type === 'laptop' ? '💻' : it.type === 'speaker' ? '🔊' : '📦'}</div>
                <div class="cart-item-details">
                    <h4 class="cart-item-name">${it.name}</h4>
                    <p class="cart-item-price">${formatUSD(dayPrice)}/день &times; <strong>${days} дн.</strong></p>
                </div>
            </div>
            <div class="cart-item-controls">
                <button class="cart-qty-btn" data-idx="${idx}" data-change="-1">−</button>
                <span class="cart-days-label">${days} дн.</span>
                <button class="cart-qty-btn" data-idx="${idx}" data-change="1">+</button>
            </div>
            <div class="cart-item-total"><span>${formatUSD(itemTotal)}</span></div>
            <button class="cart-item-remove" data-idx="${idx}">🗑️</button>
        `;
        list.appendChild(div);
    });
    document.getElementById('cartItemCount').textContent = items;
    document.getElementById('cartTotalPrice').textContent = formatUSD(total);
    // attach events
    list.querySelectorAll('.cart-qty-btn').forEach(btn => btn.addEventListener('click', () => {
        const idx = Number(btn.dataset.idx);
        const change = Number(btn.dataset.change);
        const cart2 = JSON.parse(localStorage.getItem('pc-company-cart') || '[]');
        if (cart2[idx]) {
            cart2[idx].days = Math.max(1, (cart2[idx].days || 1) + change);
            cart2[idx].totalPrice = cart2[idx].dayPrice * cart2[idx].days;
            localStorage.setItem('pc-company-cart', JSON.stringify(cart2));
            renderCartItems();
        }
    }));
...
function updateAuthUI() {
    ...
    if (isLoggedIn && currentUser) {
        try {
            ...
            authLinks.innerHTML = `
                <img src="${avatarSrc}" class="header-avatar" id="headerAvatar" alt="Аватар" title="Мій профіль">
                <span class="auth-link welcome-text">Вітаємо, ${user.name}!</span>
                <button class="auth-link profile-link" id="profileOpenBtn">👤 Профіль</button>
...
function openAvatarModal() {
            ...
            <div class="avatar-modal-body">
                <div>
                    ...
                    <div class="avatar-preview-wrap">
                        <img src="${currentAvatar}" class="avatar-preview-large" id="avatarPreviewLarge" alt="Preview" onerror="this.src='data:image/svg+xml,<svg xmlns=&quot;http://www.w3.org/2000/svg&quot; viewBox=&quot;0 0 50 50&quot;><text y=&quot;1em&quot; font-size=&quot;40&quot;>🤖</text></svg>'">
                    </div>
                </div>
                <div>
    ...
    uploadArea.addEventListener('drop', (e) => { e.preventDefault(); uploadArea.style.borderColor = ''; if (e.dataTransfer.files[0]) handleAvatarFile(e.dataTransfer.files[0], modal); });
...
function handleAvatarFile(file, modal) {
    ...
    reader.onload = (e) => { _avatarSelected = e.target.result; document.getElementById('avatarPreviewLarge').src = _avatarSelected; modal.querySelectorAll('.avatar-preset-item').forEach(i => i.classList.remove('selected')); window.showToast('📸 Фото завантажено!', 'info'); };
