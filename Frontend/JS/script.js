...
function renderCartItems(){ const list=document.getElementById('cartItemsList'); const cart=JSON.parse(localStorage.getItem('pc-company-cart')||'[]'); if(!list) return; if(cart.length===0){ list.innerHTML='<p class="empty-cart">🛒 Кошик пустий</p>'; document.getElementById('cartItemCount').textContent='0'; document.getElementById('cartTotalPrice').textContent='0 $'; return; } list.innerHTML=''; let total=0; let items=0; cart.forEach((it,idx)=>{ const price=parsePrice(it.price)||0; const itemTotal=price*it.quantity; total+=itemTotal; items+=it.quantity; const div=document.createElement('div'); div.className='cart-item'; div.style.animationDelay = (idx * 0.08) + 's'; div.innerHTML=`<div class="cart-item-info"><div class="cart-item-icon">${it.type==='desktop'?'🖥️':'💻'}</div><div class="cart-item-details"><h4 class="cart-item-name">${it.name}</h4><p class="cart-item-price">${formatUSD(price)}</p></div></div><div class="cart-item-controls"><button class="cart-qty-btn" data-idx="${idx}" data-change="-1">−</button><input type="number" class="cart-qty-input" value="${it.quantity}" min="1" data-idx="${idx}"><button class="cart-qty-btn" data-idx="${idx}" data-change="1">+</button></div><div class="cart-item-total"><span>${formatUSD(itemTotal)}</span></div><button class="cart-item-remove" data-idx="${idx}">🗑️</button>`; list.appendChild(div); }); document.getElementById('cartItemCount').textContent = items; document.getElementById('cartTotalPrice').textContent = formatUSD(total);
  // attach events
